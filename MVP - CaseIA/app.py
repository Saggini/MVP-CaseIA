...
def limpar_texto(prompt):
    ...
    # Ignorar qualquer texto antes de "Caso de Teste 1:"
    prompt = re.sub(r"^.*?(Caso de Teste 1:)", r"\1", prompt, flags=re.DOTALL)
    ...
    # Remover "**" depois de "Cenário Positivo:"
    prompt = re.sub(r"(Cenário Positivo:)\s*\*\*", r"\1", prompt, flags=re.IGNORECASE)
    # Remover "**" depois de "Cenário Negativo:"
    prompt = re.sub(r"(Cenário Negativo:)\s*\*\*", r"\1", prompt, flags=re.IGNORECASE)
    ...
    # Remover ** em qualquer parte do texto
...
def generate_tests():
    try:
        ...
        if system_type == "Web":
            ...
            3. Passos:: Descreva o fluxo no formato BDD com:
            ...
            prompt = f"""
            Você é um especialista em testes de APIs RESTful. Com base nos critérios de aceitação fornecidos, crie casos de teste no formato BDD (Dado que, Quando, Então) para validar endpoints de APIs.

            Cada caso de teste deve incluir:
            Título: No formato "Caso de Teste X: Nome do Endpoint ou Validação"
            Descrição: O que será validado (ex.: retorno correto, códigos de status, estrutura JSON, headers, autenticação).
            Passos:
            Dado que: Descreva o estado do sistema (ex.: usuário autenticado, base de dados populada).
            Quando: A requisição for feita (GET, POST, PUT, DELETE).
            Então: O sistema deve retornar o esperado (status 200, mensagem, objeto JSON válido etc.).

            Regras para elaboração dos casos:
            Validar códigos de resposta HTTP.
            Testar valores obrigatórios e inválidos no payload.
            Verificar respostas com headers corretos.
            Validar tempo de resposta e limites de requisições (rate limit).
            Incluir testes de autenticação e autorização (token JWT, OAuth etc.).

            Cenários alternativos:

            Positivos: requisições válidas.
            Negativos: requisições inválidas, dados malformados.
            Borda: dados muito grandes, listas vazias, valores limite (ex.: id=0, id=9999999).
            ...
            prompt = f"""
            Você é um especialista em testes de aplicações mobile. Crie casos de teste no formato BDD para validar funcionalidades em apps mobile nativos ou híbridos.

            Cada caso de teste deve incluir:

            Título: No formato "Caso de Teste X: Nome da Tela ou Ação"
            Descrição: O que será validado (navegação, gesto de toque, swipe, uso offline, etc.).
            Passos:
            Dado que: Estado do app (usuário logado, tela aberta, modo offline).
            Quando: O usuário interagir com o app (toque, rolagem, envio de formulário).
            Então: Deve ocorrer o comportamento esperado (mudança de tela, feedback visual, animação, etc.).

            Regras para elaboração dos casos:
            Validar botões, campos de entrada, scroll, gestures (swipe, pinch).
            Testar notificações push e permissões de sistema (GPS, câmera).
            Validar modo escuro/claro.
            Testar funcionamento offline e sincronização online.
            Considerar diferentes tamanhos de tela e densidade de pixels.

            Cenários alternativos:

            Positivos: fluxo padrão completo.
            Negativos: perda de conexão, permissão negada.
            Borda: rotação de tela no meio da operação, entrada de dados muito longos.
            ...
            prompt = f"""
            Você é um especialista em testes de aplicações desktop. Crie casos de teste no formato BDD para validar a interface e o comportamento de sistemas desktop.

            Cada caso de teste deve incluir:
            Título: No formato "Caso de Teste X: Nome da Funcionalidade ou Tela"
            Descrição: O que será testado (atalhos, abertura de janelas, menus, arquivos, etc.).

            Passos:
            Dado que: O sistema está instalado e rodando.
            Quando: O usuário realiza uma ação com mouse ou teclado.
            Então: O sistema responde conforme esperado (janela abre, arquivo é salvo, notificação aparece).

            Regras para elaboração dos casos:
            Validar menus, barras de ferramentas, botões, atalhos de teclado.
            Testar salvar, abrir, exportar arquivos.
            Validar comportamento em múltiplos monitores e diferentes resoluções.
            Testar acessibilidade com navegação por teclado, leitores de tela.

            Cenários alternativos:

            Positivos: fluxo normal de uso.
            Negativos: falha ao salvar, tentativa de abrir arquivo corrompido.
            Borda: abrir múltiplas janelas, minimizar e restaurar com ações em execução.
...
def export_pdf():
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 10, "Casos de Teste - Página por Caso", ln=True, align="C")
            self.ln(5)
    ...
    # ✅ Registrar a fonte com os 3 estilos
    pdf.add_font("DejaVu", "", font_path, uni=True)   # normal
    ...
    if not generated_test_results:
        ...
        pdf.cell(0, 10, "Nenhum caso de teste foi gerado.", ln=True)
    else:
        for index, test_case in enumerate(generated_test_results, start=1):
            pdf.add_page()

            title = test_case.get("Title", f"Caso de Teste {index}")
            description = test_case.get("Description", "Descrição não fornecida.")
            step_action = test_case.get("Step Action", "")
            step_expected = test_case.get("Step Expected Result", "")
            ...
            pdf.set_font("DejaVu", "B", 11)
            pdf.cell(0, 10, "Passos:", ln=True)
            pdf.set_font("DejaVu", "", 11)

            all_steps = []
            if step_action:
                all_steps += [s.strip() for s in step_action.split("\n") if s.strip()]
            if step_expected:
                all_steps += [s.strip() for s in step_expected.split("\n") if s.strip()]

            for step in all_steps:
                pdf.multi_cell(0, 10, f"- {step}")

    file_path = os.path.join(app.config["UPLOAD_FOLDER"], "test_cases_formatado.pdf")
